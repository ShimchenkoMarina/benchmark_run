...
for dir in $(find $COMMIT/ -mindepth 2 -maxdepth 3 -type d -links 2); do
	...
	echo $dir
        ...
        cat ${input_dir}/${i}.txt  | grep "Power consumption of package" | cut -d " " -f 6 >> ${raw_dir}/energy/${i}.txt
        ...
        cat ${input_dir}/${i}.txt | grep "Execution time"| cut -d " " -f 3  >> ${raw_dir}/perf/${i}.txt
        ...
        cat ${input_dir}/${i}.txt | grep "metered tail latency" | cut -d ' ' -f 13 >> ${raw_dir}/latency/${i}.txt
