...
for dir in $(find $COMMIT/ -mindepth 2 -maxdepth 3 -type d -links 2); do
    ...
    for i in $(seq 1 $count_files); do
        ...
        cat ${raw_dir}/${i}.txt | grep "Power consumption of dram" | cut -d ' ' -f 5 >> ${raw_dir}/energy_dram/${i}.txt
        cat ${raw_dir}/${i}.txt | grep "Power consumption of dram" | cut -d ' ' -f 5 >> ${raw_dir}/av_power_dram/${i}.txt
        cat ${raw_dir}/${i}.txt | grep "Power consumption of cpu" | cut -d ' ' -f 5 >> ${raw_dir}/energy_cpu/${i}.txt
        cat ${raw_dir}/${i}.txt | grep "Power consumption of cpu" | cut -d ' ' -f 5 >> ${raw_dir}/av_power_cpu/${i}.txt
        cat ${raw_dir}/${i}.txt | grep "Power consumption of package" | cut -d ' ' -f 5 >> ${raw_dir}/energy_pack/${i}.txt
        cat ${raw_dir}/${i}.txt | grep "Power consumption of package" | cut -d ' ' -f 5 >> ${raw_dir}/av_power_pack/${i}.txt
